...
module_name = "Socialhunt: Find Usernames Across Social Networks"
__version__ = "0.14.3"


class SocialhuntFuturesSession(FuturesSession):
    def request(self, method, url, hooks=None, *args, **kwargs):
        ...
        return super(SocialhuntFuturesSession, self).request(method,
                                                           url,
                                                           hooks=hooks,
                                                           *args, **kwargs)
...
def CheckForParameter(username):
    ...
    if exist it means that Socialhunt is looking for more multiple username'''
    return ("{?}" in username)
...
def Socialhunt(username, site_data, query_notify,
             tor=False, unique_tor=False,
             proxy=None, timeout=60):
    """Run Socialhunt Analysis.
    ...
    session = SocialhuntFuturesSession(max_workers=max_workers,
...
def main():
    ...
    # Check for newer version of Socialhunt. If it exists, let the user know about it
    try:
        r = requests.get(
            "https://raw.githubusercontent.com/CVEDB/socialhunt/master/socialhunt/socialhunt.py")
        ...
        if remote_version != local_version:
            print("Update Available!\n" +
                  f"You are running version {local_version}. Version {remote_version} is available at https://github.com/CVEDB/Socialhunt")
